...
begin
  if CurStep = ssPostInstall then begin
    ...
    s[1] := '"' + ExpandConstant('{app}\xcashd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
...
Name: "{group}\Utilities\Monero Daemon"; Filename: "{app}\xcashd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM xcashd.exe /T /F"
